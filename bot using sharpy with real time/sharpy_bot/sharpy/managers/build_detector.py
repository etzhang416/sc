...
class BuildDetector(ManagerBase):
    ...
    def _protoss_rushes(self):
        ...
        if len(self.cache.enemy(UnitTypeId.NEXUS)) > 1:
            ...
            if self.ai.time >= 2 * 60 + 40 and self.rush_build != EnemyRushBuild.EarlyExpand and \
                    ...
                    and self.rush_build != EnemyRushBuild.CannonRush:
        ...
        if 2 * 60 + 15 <= self.ai.time <= 3 * 60:
            ...
            if self.cache.enemy(UnitTypeId.PYLON).amount < 2:
                ...
                if self.cache.enemy(UnitTypeId.STARGATE).amount >= 1:
        ...
        if 2 * 60 + 45 < self.ai.time <= 4 * 60 + 20 and \
    ...
    def _zerg_rushes(self):
        ...
        if self.rush_build == EnemyRushBuild.Macro:
            ...
            if 75 <= self.ai.time <= 95:
                if self.cache.enemy(UnitTypeId.HATCHERY):
                    if self.cache.enemy(UnitTypeId.HATCHERY).first.is_active:
                        return self._set_rush(EnemyRushBuild.NySwarm)
            ...
            if self.ai.time > 95:
                return self._set_rush(EnemyRushBuild.EcoExpand)
        if self.rush_build == EnemyRushBuild.NySwarm:
    ...
    def _terran_rushes(self):
        ...
        if self.rush_build == EnemyRushBuild.ProxyMarauders:
            if self.cache.own(UnitTypeId.IMMORTAL).amount >= 2:
                return self._set_rush(EnemyRushBuild.TerranMacro)
        if self.rush_build == EnemyRushBuild.ProxyMarine:
        ...
        if len(self.cache.enemy(
                [UnitTypeId.COMMANDCENTER, UnitTypeId.ORBITALCOMMAND, UnitTypeId.PLANETARYFORTRESS])) > 1:
            if 1 * 60 + 40 <= self.ai.time <= 2 * 60:
                self._set_rush(EnemyRushBuild.EarlyExpand)
                return
            if 2 * 60 <= self.ai.time <= 2 * 60 + 20:
                self._set_rush(EnemyRushBuild.TerranMacro)
                return
        ...
        if 1 * 60 + 39 <= self.ai.time <= 1 * 60 + 41:
